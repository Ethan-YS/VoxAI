...
- **不主打 AI 朗读功能（v1）**——TTS / MCP `speak` 在 v1 里实现，但作为辅助。主战场是 ASR 输入侧。详见 DECISIONS DR-015。
- **不做会议模式（v1）**——依赖 whisperx 离线转录 + pyannote 声纹分离，迁移到 WhisperKit 工作量 ≥ 2 周。详见 DECISIONS DR-004。
- **不做声纹分离（v1）**——同上依赖链。
- **不绑定特定 TTS 服务方**——Cloud TTS 走"用户自带 OpenAI API key + OpenAI 兼容协议"。**不打包 edge-tts**，因为它逆向 Microsoft 未公开 API，会同时踩 App Store Guideline 2.5.1（public APIs only）+ 5.2.2（third-party authorization）+ GPL-3.0 许可证三道雷。详见 DECISIONS DR-008 / LICENSE_AUDIT。
- **不打包 Python 后端**——Sandbox 禁止 spawn 用户路径下的 Python；纯 Swift 实现是 App Store 路线的硬约束。详见 DECISIONS DR-003。
- **不做本地中文 TTS（Qwen3-TTS / mlx-audio）**——无 Swift 包装，且 Apple 系统中文 TTS 已能覆盖（且 TTS 不是主推）。
- **不做本地英文 TTS（Chatterbox）**——同理。
- **不做"自动 ⌘V paste 到当前活动窗口"（v1）**——体验最爽，但需要 Accessibility 权限，首次启动用户要在系统隐私设置手动加 VoxAI，授权摩擦大。v1.x 评估，详见 DR-020。
- **不做 SFSpeechRecognizer on-device 开关（v1）**——v1 主打体验/识别准确率，默认走 Apple 服务器识别。隐私政策诚实告知。详见 DR-018。
- **不替代 VoxSage**——双轨发布，旧仓继续维护，新仓只做 ASR 聚焦的子集。
- **不做录音 app / 笔记 app**——定位是"开发者的语音输入工具"，不是独立的语音工具。
- **不做 SaaS / 订阅模式（v1）**——v1 免费，最大化用户基数。详见 DECISIONS DR-016。

## 目标用户 / 使用者

**主要用户**：
- **使用 Claude Code / Cursor / 其他 AI 编程工具的开发者**——希望用语音写 prompt 取代部分键盘输入
...
**用户故事**：
> 我在 Claude Code 里写代码，想让它解释这段函数。我点 VoxAI 浮窗的录音按钮，对着麦克风说"解释一下这个文件第 42 行那个递归，特别是 base case 那部分逻辑"。停下后文字已经在剪贴板里。切到 Claude，⌘V，回车。整个过程我的手没离开过键盘旁边。
