...
- **App Sandbox 全程开启**
- **v1.0 entitlements 极简**：只 `app-sandbox` + `device.audio-input`（DR-022 砍 MCP server / DR-021 砍 Cloud TTS 后不需要 `network.*`）
- **v1.0 主推"用嘴编程"** —— ASR + 自动复制剪贴板。**砍掉**：AI 朗读 TTS（DR-021）/ MCP HTTP server（DR-022）/ 语言切换 UI（DR-023）/ 自动 paste（DR-024）。详见 brain/DECISIONS.md
- **v1 不做会议模式** — 留给 v1.1
- **目标市场是中文** — 默认 zh-CN，App Store 描述 / 关键词 中文聚焦
