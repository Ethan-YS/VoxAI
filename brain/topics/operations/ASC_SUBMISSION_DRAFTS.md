...
ASC 现行流程在**选定 build 之后**，build 行右侧"⚠️ 缺少出口合规证明"旁的 **"管理"** 链接里跳出 "App 加密文稿" modal——**不是 Submit 时才跳**。

modal 问的是**"你的 App 采用了哪种类型的加密算法？"**（四选一）：

| 选项 | 含义 | VoxAI 选不选 |
|---|---|---|
| 1. 专有或未被国际标准主体（IEEE、IETF、ITU 等）视为标准的加密算法 | 自己实现的非标准算法 | ❌ |
| 2. 代替 / 与系统加密并用的标准加密算法 | 自己实现的标准算法（替代/补充 Apple 系统加密） | ❌ |
| 3. 兼用 1 和 2 | — | ❌ |
| 4. **不属于上述的任意一种算法** | App 自己没用任何加密算法 | ✅ |

**VoxAI 选 4 的理由**：
- v1.0 没 import 任何 crypto / TLS 框架
- DR-021 砍 Cloud TTS（本来要走 HTTPS）
- DR-022 砍 MCP HTTP server
- 唯一可能的网络是 SFSpeechRecognizer，加密发生在 Apple 系统层不归 App 声明

选 4 + 保存后 build 行的 ⚠️ 消失，"添加以供审核"按钮变可点。**不会再跳 ECCN / BIS 申报问卷**。

### Info.plist 标记（推荐 v1.x 补）

可以在项目里加 `ITSAppUsesNonExemptEncryption = NO` 让 ASC 永久跳过这个 modal（每次新版本不再问）：
...
v1.0 没加——手动答了一次 Option 4。v1.x 时 Sage 顺手补上。

### v1.x 警告：加 Cloud TTS / 网络功能时必须重走

如果 v1.x 加回 Cloud TTS（OpenAI HTTPS）/ MCP server / 任何网络 → **不再选 Option 4**。届时即便用的是系统标准 TLS（URLSession），也通常按 Option 2 + 走 BIS 豁免路径声明。重新评估 §11。
...
3. 点"添加构建版本"→ radio 选刚处理好的 build → 点"完成"
4. **build 行右侧会出现 ⚠️ "缺少出口合规证明" + "管理" 链接** → 点管理 → 走 §11 实战表（v1.0 选 Option 4）→ 保存 → ⚠️ 消失
5. 点页面右上角"保存"持久化 build 选择
6. 左 sidebar 切到"App 隐私"→ 右上角点"**发布**"（publish 隐私设置到产品页）
7. 切回"1.0.0 准备提交"→ 右上角"**添加以供审核**"创建草稿
8. 右侧抽屉底部蓝色"**提交以供审核**"才是真正 Submit（不可逆）
...
## 13.5 v1.0.0 提交记录（2026-05-06）

> 永久 reference。Apple 拒了 / 通过了，submission ID 都用得上。
...
| Bundle ID | `com.ethanys.voxai` |
| Build | `1.0 (1)` |
| .pkg 路径 | `build/Export-AppStore/VoxAI.pkg` (3.6 MB) |
| 签名 | Apple Distribution: SANG YI (YNMBJ5H736) |
| Transporter 交付时间 | 2026-05-06 上午 8:47 |
| ASC 处理完成时间 | 2026-05-06 上午 8:48（约 1 分钟，比 §13 估的 5-30 分钟快） |
| 出口合规答法 | Option 4（不属于上述任意一种算法） |
| App 隐私 publish | 2026-05-06（由"桑 伊"） |
| 提交 ID | `af17f3c1-f264-4ad3-8bdd-5a84ea339a32` |
| 提交时间 | 2026-05-06 上午 9:12 |
| 提交者 | 伊桑 (Rebecca, `hrebeccaqy@icloud.com`) |
| **审核通过 / 可分发** | **2026-05-06**（同一天，< 12h） |
| **App Store 索引到** | **2026-05-06** |
| **产品页 URL** | https://apps.apple.com/cn/app/voxai-用嘴编程/id6766570591 |
| **canonical URL（区域中性）** | https://apps.apple.com/app/id6766570591 |

提交后在 ASC → 分发 → App 审核 → macOS 提交 页可看到状态。

**审核速度数据点**：提交 → 通过 < 12h，远快于 Apple 官方"1-3 天"预期。单点样本，v1.x 提交时不要 over-fit 此基线。
